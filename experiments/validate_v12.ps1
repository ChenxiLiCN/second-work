...
try {
    $rows=foreach($group in (Import-Csv experiments/v11/real-correctness.csv | Group-Object dataset,meta_path,epsilon,mu)) {
        ...
        $out="experiments/v12/validation/$($case.dataset)/$stem/$($case.epsilon)-$($case.mu)"
        New-Item -ItemType Directory -Force -Path $out | Out-Null
        $bri="experiments/v11/benchmark/$($case.dataset)/base.bri"
        $before=(Get-FileHash $bri).Hash
        $lines=& ./build-cmake/bin/bri_query_blocks.exe $bri $case.meta_path $case.epsilon $case.mu $out 2>&1
        $code=$LASTEXITCODE
    ...
    $rows | Export-Csv -NoTypeInformation -Encoding utf8 -LiteralPath experiments/v12/real-correctness.csv
