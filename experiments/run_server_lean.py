#!/usr/bin/env python3
"""One pass per DBLP: lean workspace versus unchanged single-pass predicate."""
import csv
import json
import os
...
def main():
    ...
    run = Path(tempfile.mkdtemp(prefix=time.strftime('%Y%m%d-%H%M%S-lean-'), dir=root))
    ...
    try:
        ...
        env = dict(version='lean_v1', source=str(source), inputs=inputs,
            ...
            isolation='Both current queries read the SAME basic index. Both use mode-9 core certificates and the same single-pass/cache policy. Only activation clearing and duplicate-bound checks differ. No enhanced degrees, new index, profiler or parameter sweep.',
        ...
        env['source_sha256'][str(Path(__file__).relative_to(ROOT))] = digest(Path(__file__))
        ...
        checked(['cmake', '--build', build, '--parallel', jobs, '--target',
                 'bri_query_core_lean', 'bri_query_core_single_pass', 'verify_adaptive_fli'], run / 'build.log')
        ...
        for name in DATASETS:
            case = run / name
            case.mkdir()
            for label, exe in [('control', 'bri_query_core_single_pass'), ('lean', 'bri_query_core_lean')]:
                ...
                try:
                    ...
                    config_ok &= (metric.get('block_mode') == ('10' if label == 'control' else '11') and
                                  metric.get('lean_workspaces') == ('0' if label == 'control' else '1') and
                                  metric.get('single_pass') == '1' and metric.get('witness_counts_built') == '0' and metric.get('witness_bound_checks') == '0')
            ...
            invariant_keys = ('certified_blocks', 'block_core_vertices', 'adaptive_hits', 'adaptive_misses',
                'adaptive_evictions', 'adaptive_posting_entries', 'adaptive_streaming_entries',
                'single_pass_complete', 'single_pass_partial', 'activation_calls', 'exact_similarity_checks',
                'candidate_posting_entries_read')
            if len(pair) == 2 and any(k not in pair[0] or k not in pair[1] or pair[0][k] != pair[1][k] for k in invariant_keys):
                for r in pair:
                    r['valid'] = 0
                    r.pop('online_compute_s', None)
                    r.pop('speedup_vs_historical_hinscan', None)
                errors.append(f'{name}: cache/scanning policy changed')
        ...
        keys = ('dataset', 'variant', 'valid', 'online_compute_s', 'historical_hinscan_compute_s',
            ...
            'candidate_posting_entries_read', 'exact_similarity_checks', 'activation_calls',
            'activation_full_words_written', 'activation_sparse_words_cleared', 'streaming_bound_evaluations')
