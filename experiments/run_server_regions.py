#!/usr/bin/env python3
"""One pass per DBLP: whole-region completion versus unchanged mode 9."""
import csv
import json
import os
...
def main():
    ...
    run = Path(tempfile.mkdtemp(prefix=time.strftime('%Y%m%d-%H%M%S-regions-'), dir=root))
    ...
    try:
        ...
        env = dict(version='regions_v1', source=str(source), inputs=inputs,
            repeats=1, cache_mib=32, fallback_mode=9,
            ...
            counters='avoided_degree_merge_entries counts raw union-loop entries, not unique edges. Candidate/check counts compare residual execution with control; they need not decrease monotonically because processing order/cache behavior may change.',
        ...
        checked(['cmake', '--build', build, '--parallel', jobs, '--target',
                 'bri_query_regions', 'bri_query_core_connectivity', 'verify_adaptive_fli'], run / 'build.log')
        ...
        for name in DATASETS:
            case = run / name
            case.mkdir()
            for label, exe in [('control', 'bri_query_core_connectivity'), ('regions', 'bri_query_regions')]:
                ...
                try:
                    ...
                    config_ok &= (metric.get('block_mode') == '9' if label == 'control' else
                                  metric.get('algorithm') == 'region_completion_v1' and metric.get('fallback_block_mode') == '9')
                    ...
                    if valid:
                        row['online_compute_s'] = (float(metric['on_demand_fli_build_ms']) + float(metric['query_call_ms'])) / 1000 if label == 'control' else float(metric['online_compute_ms']) / 1000
        ...
        keys = ('dataset', 'variant', 'valid', 'online_compute_s', 'historical_hinscan_compute_s',
            'speedup_vs_historical_hinscan', 'region_proof_ms', 'completed_core_vertices',
            'completed_noncore_vertices', 'residual_vertices', 'avoided_degree_merge_entries',
            'residual_candidate_posting_entries', 'residual_exact_similarity_checks')
