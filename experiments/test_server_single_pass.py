...
import run_server_single_pass as runner


class SinglePassRunnerTests(unittest.TestCase):
    def exercise(self, fault=None):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch.dict(os.environ, RESULT_ROOT=str(root), INDEX_SEARCH_ROOT=str(root)), \
                ...
                code = runner.main()
            run = next(p for p in root.glob('*-single-pass-*') if p.is_dir())
